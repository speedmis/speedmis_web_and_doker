...
$full_siteID = "localho2";			//--- 고객등록ID, 3~8자, 알파벳으로 시작, 알파벳또는숫자
...
$base_root = "/var/www/html";			//--- 루트의 물리적 경로를 넣되, 마지막에 슬래시는 넣지 말것, ex: d:/webroot/web_speedmis_v6
...
$allKill_pw = "1234!!00";			//--- 비상시, allKill_pw 의 값을 넣으면 어느 ID 로든지 로그인 됨, 단 사용자가 텔레그램 설정했으면 통보됨.
...
$DbServerName = "cloud.speedmis.com";			//--- DB서버이름 또는 IP
$base_db = "speedmis_db_demo";			//--- 기본 데이터베이스명
$DbID = "speedmis_db_demo";			//--- DB서버 id
$DbPW = "1234!!00";			//--- DB서버 pw
...
/* 리눅스 웹서버에서 1차 DB 서버 MSSQL 접속설정 */
if ($MS_MJ_MY == 'MY') {
    $externalDB['1st'] = "MY(@)$DbServerName(@)$base_db(@)$DbID(@)$DbPW";
} else {
    $driverOptions = [PDO::ATTR_CASE => PDO::CASE_NATURAL, PDO::ATTR_TIMEOUT => 600, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION];
    $database = new PDO("dblib:host=$DbServerName;dbname=$base_db;charset=utf8", "$DbID", "$DbPW", $driverOptions);
}

include __DIR__ . "/top_addLogic.php";
