...
*/
function move_station(p_this, p_move) {
	idx = $(p_this).attr('idx');
...
function add_station(p_this, p_add, p_depth) {
	add_name = prompt('추가할 부서명을 입력하세요.', '');
...
function open_info(p_this) {
	...
	idx = $(p_this).attr('idx');
	url = 'index.php?RealPid=speedmis000114&idx='+idx+'&ActionFlag=modify';
	parent_popup_jquery(url, teamName+' 수정', 600, 400, true);
}
...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='g4name') {
		...
		rValue0 = `
<a onclick="add_station(this,'down',1);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 부서추가">+</a>
`;
...
`;
		...
		if(p_dataItem['AutoGubun']!='0000' && Len(p_dataItem['AutoGubun'])==4 ) {
			rValue1 = `
<a onclick="move_station(this,'top');" idx="`+p_dataItem['idx']+`" class="k-button hide"`+iif(p_dataItem['AutoGubun']=='0001',' disabled','')+` title="맨위로 이동">↑</a>
<a onclick="move_station(this,'up');" idx="`+p_dataItem['idx']+`" class="k-button"`+iif(p_dataItem['AutoGubun']=='0001',' disabled','')+` title="한칸 위로 이동">▲</a>
<a onclick="move_station(this,'down');" idx="`+p_dataItem['idx']+`" class="k-button" title="한칸 아래로 이동">▼</a>
<a onclick="move_station(this,'bottom');" idx="`+p_dataItem['idx']+`" class="k-button hide" title="맨 아래로 이동">↓</a>
`;
		}
		...
		if(p_dataItem['AutoGubun']!='0000' && Len(p_dataItem['AutoGubun'])>=4 && Len(p_dataItem['AutoGubun'])<=8) {
			rValue0 = `
<a onclick="add_station(this,'down',2);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 부서추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
`;
		...
		if(Len(p_dataItem['AutoGubun'])==8) {
			rValue1 = `
<a onclick="move_station(this,'top');" idx="`+p_dataItem['idx']+`" class="k-button hide"`+iif(Right(p_dataItem['AutoGubun'],4)=='0001',' disabled','')+` title="맨위로 이동">↑</a>
<a onclick="move_station(this,'up');" idx="`+p_dataItem['idx']+`" class="k-button"`+iif(Right(p_dataItem['AutoGubun'],4)=='0001',' disabled','')+` title="한칸 위로 이동">▲</a>
<a onclick="move_station(this,'down');" idx="`+p_dataItem['idx']+`" class="k-button" title="한칸 아래로 이동">▼</a>
<a onclick="move_station(this,'bottom');" idx="`+p_dataItem['idx']+`" class="k-button hide" title="맨 아래로 이동">↓</a>
`;
		}
		...
		if(Len(p_dataItem['AutoGubun'])>=8 && Len(p_dataItem['AutoGubun'])<=12) {
			rValue0 = `
<a onclick="add_station(this,'down',3);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 부서추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
function addLogic_treat() {
	...
	if($question=="move") {
		$sql = " select SortG2,SortG4,SortG6,SortG8,SortG10,depth from MisStation where num='$p_idx' ";
		$r = allreturnSql($sql);
		$depth = $r[0]['depth'];
		$SortG2 = $r[0]['SortG2'];
		$SortG4 = $r[0]['SortG4'];
		...
		$sql = " 
		update MisStation set SortG2='$SortG2',SortG4='$SortG4',SortG6='$SortG6',SortG8='$SortG8',SortG10='$SortG10' where num='$p_idx';
		EXECUTE MisStation_Ordering_Proc;
		";
		//echo $sql;
		execSql($sql);
		echo 'success';
		...
		$sql = " select SortG2,SortG4,SortG6,SortG8,SortG10,upidx,depth from MisStation where num='$p_idx' ";
		$r = allreturnSql($sql);
		...
		$sql = " 
		insert into MisStation (StationName, upidx, SortG2, SortG4, SortG6, SortG8, SortG10, depth, wdater) 
