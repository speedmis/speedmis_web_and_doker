...
function viewLogic_afterLoad_continue() {
	...
	cont = $('div#contents')[0].innerHTML;
	cont = replaceAll(cont, '{RUN}', '<a role="button" class="k-button k-button-icontext" id="btn_run">구현코드 결과값 확인</a>');
	$('div#contents')[0].innerHTML = cont;
	$('a#btn_run').click( function() {
			eval($('textarea#phpcode').val());
			alert(result);
	});
...
function addLogic_treat() {
    ...
    $sql = requestVB("sql");
	$sql = decode_firewall($sql);
    $sm = requestVB("sm");


	//아래는 값에 따라 mysql 서버를 통해 알맞는 값을 출력하여 보냅니다.
    if(InStr($sql, "select ")>0 && InStr($sql, " where ")>0) {
		if($sm=='m') {
			$result = allturnSql($sql);
			echo JSON.encode($result,true);
		} else {
			$result = onlyOnereturnSql($sql);
			echo $result;
		}
...
function jsonUrl_index() {
	...
	global $jsonUrl, $RealPid;
	
	//아래에 대상사이트를 넣습니다.
	if($RealPid=='speedmis003029') $jsonUrl = "https://www.speedmis.com/_mis/";
}
//end jsonUrl_index
