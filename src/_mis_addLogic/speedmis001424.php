...
function list_json_init() {
    ...
    if($flag=='read') { 
        if($app=='자동정렬' || $grid_load_once_event=='xxxN') {
            $appSql = "select count(idx) from MisDocFolder where sortG2='0' and useflag=1 ";
            $cnt = onlyOnereturnSql($appSql);
            if($cnt==1) {
                if($MS_MJ_MY=='MY') {
                    $appSql = "call MisDocFolder_Ordering_Proc();";
                } else {
                    $appSql = "EXECUTE MisDocFolder_Ordering_Proc";
                }
                if(execSql($appSql)) {
                    $resultCode = "success";
...
//exit;
	...
	$AutoGubun = onlyOnereturnSql("select AutoGubun from MisDocFolder where idx='" . $updateList["upidx"] . "'");
