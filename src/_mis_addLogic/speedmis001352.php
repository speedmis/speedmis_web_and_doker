...
특이사항 : 
support table 기준으로 제작됨. speedmis 사이트에서만 사용함.
...
function pageLoad() {
    ...
    if($ActionFlag=="list" || $ActionFlag=="view") {
        ...
        $sql = "
        update Mis__speedsup__Comments set sel_like=0 where sel_like is null;
        update Mis__speedsup__Comments set sel_hate=0 where sel_hate is null;
        ";
...
function list_query() {
	...
	//아래는 어떤 특정한 상황에 대한 적용예입니다.
        ...
        $selectQuery = replace($selectQuery, "table_m.contents as ", "replace(table_m.contents,'http://support.speedmis.com','https://www.speedmis.com/z_support') as ");
...
function save_updateBefore() {
    ...
    if($ActionFlag=="write") {
		...
		$tempSql = "select case when MenuType='01' then g08 else (select m.g08 from Mis__speedsup__MenuList m where m.RealPid=table_m.MisJoinPid) end 
		from Mis__speedsup__MenuList table_m where RealPid='". $updateList["RealPid"] . "'";
...
function save_writeBefore() {
    ...
    if($MS_MJ_MY=='MY') {
        $sql = "select ifnull(dbalias,'') from Mis__speedsup__MenuList where RealPid='" . get_logicPid($parent_idx) . "'";
    } else {
        $sql = "select isnull(dbalias,'') from Mis__speedsup__MenuList where RealPid='" . get_logicPid(GubunIntoRealPid($parent_gubun)) . "'";
    }
    ...
    if($MS_MJ_MY=='MY') {
        //검증필요
        $parent_field = onlyOnereturnSql("select Grid_Select_Field from Mis__speedsup__MenuList_Detail where RealPid=
        (select case when MenuType='01' then RealPid else MisJoinPid end from Mis__speedsup__MenuList 
        where RealPid='" . $updateList["RealPid"] . "')
        and (SortElement=1 and Grid_Columns_Width>-1 or SortElement>=2) order by SortElement limit 1");

        $sql_next = " 
        update Mis__speedsup__Comments set refidx=idx where idx=$newIdx or refidx is null;
        ";
        if($parent_MS_MJ_MY=='MY') $sql_next = $sql_next . " 
        select @cnt=count(*) from Mis__speedsup__Comments 
        ...
        $parent_field = onlyOnereturnSql("select top 1 Grid_Select_Field from Mis__speedsup__MenuList_Detail where RealPid=
        (select case when MenuType='01' then RealPid else MisJoinPid end from Mis__speedsup__MenuList 
        where RealPid='" . $updateList["RealPid"] . "')
        ...
        declare @cnt int select @cnt=count(*) from Mis__speedsup__Comments 
        where RealPid='" . $updateList["RealPid"] . "' and midx='" . $updateList["midx"] . "' and useflag=1 
...
function save_deleteBefore() {
    ...
    if($MS_MJ_MY=='MY') {
        ...
        $parent_table = onlyOnereturnSql("select g08 from Mis__speedsup__MenuList where RealPid=
        (select case when MenuType='01' then RealPid else MisJoinPid end from Mis__speedsup__MenuList 
        where RealPid='" . GubunIntoRealPid($parent_gubun) . "')");

        $sql_next = " 
        update Mis__speedsup__Comments set useflag=0 from Mis__speedsup__Comments m, (
            select refidx from Mis__speedsup__Comments where refidx in 
            (select refidx from Mis__speedsup__Comments where idx=refidx and useflag=0 and concat(midx,RealPid)=(select concat(midx,RealPid) 
            from Mis__speedsup__Comments where idx in ($deleteList)))
            ) d where m.refidx=d.refidx;

        select @cnt=COUNT(*) from Mis__speedsup__Comments where useflag=1 and midx = (
         select midx from  Mis__speedsup__Comments  where idx in ($deleteList));
        update $parent_table set comment_count=@cnt where $parent_field=(select midx from Mis__speedsup__Comments where idx in ($deleteList));
        ";
    } else {
        $parent_field = onlyOnereturnSql("select top 1 Grid_Select_Field from Mis__speedsup__MenuList_Detail where RealPid=
        (select case when MenuType='01' then RealPid else MisJoinPid end from Mis__speedsup__MenuList 
        ...
        $sql_next = " 
		update Mis__speedsup__Comments set useflag=0 from Mis__speedsup__Comments m join (
            select refidx from Mis__speedsup__Comments where refidx in 
            (select refidx from Mis__speedsup__Comments where idx=refidx and useflag=0 and convert(varchar(5),midx)+RealPid=(select convert(varchar(5),midx)+RealPid 
            from Mis__speedsup__Comments where idx in ($deleteList)))
            ) d on m.refidx=d.refidx where m.refidx=d.refidx
            
        declare @cnt int select @cnt=COUNT(*) from Mis__speedsup__Comments where useflag=1 and midx = (
         select midx from  Mis__speedsup__Comments  where idx in ($deleteList))
        update $parent_table set comment_count=@cnt where $parent_field=(select midx from Mis__speedsup__Comments where idx in ($deleteList))
        ";
