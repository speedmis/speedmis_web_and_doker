...
function pageLoad() {
    ...
    global $ActionFlag, $RealPid, $parent_idx, $idx, $table_m;
...
if exists(select * from information_schema.Columns where TABLE_NAME='$table_m' and COLUMN_NAME='numQ1주' and DATA_TYPE='float') begin

	alter table $table_m alter column 생년월일 int
	alter table $table_m alter column 생년월일 varchar(10)
	
	alter table $table_m alter column numQ1주 int
	alter table $table_m alter column numQ1주 varchar(10)

	alter table $table_m alter column numQ2주 int
	alter table $table_m alter column numQ2주 varchar(10)

	alter table $table_m alter column numQ3주 int
	alter table $table_m alter column numQ3주 varchar(10)

	alter table $table_m alter column numQ4주 int
	alter table $table_m alter column numQ4주 varchar(10)

	alter table $table_m alter column numQ5주 int
	alter table $table_m alter column numQ5주 varchar(10)

	alter table $table_m alter column numQ6주 int
	alter table $table_m alter column numQ6주 varchar(10)

	alter table $table_m alter column numQ7주 int
	alter table $table_m alter column numQ7주 varchar(10)
	
	alter table $table_m alter column numQ8주 int
	alter table $table_m alter column numQ8주 varchar(10)

	alter table $table_m alter column 등록일자 int
	alter table $table_m alter column 등록일자 varchar(10)
	
	alter table $table_m add remark nvarchar(100)
...
function list_json_init() {
    global $RealPid, $MisJoinPid, $logicPid, $parent_idx, $full_siteID, $grid_load_once_event, $table_m;
    ...
    if($flag=='read' && $selField=='' && $grid_load_once_event=='N') { 
        $sql = "

if exists(select * from $table_m where len(numQ1주)=8) begin


	update $table_m set 생년월일='' where 생년월일='0'
	update $table_m set numQ1주='' where numQ1주='0'
	update $table_m set numQ2주='' where numQ2주='0'
	update $table_m set numQ3주='' where numQ3주='0'
	update $table_m set numQ4주='' where numQ4주='0'
	update $table_m set numQ5주='' where numQ5주='0'
	update $table_m set numQ6주='' where numQ6주='0'
	update $table_m set numQ7주='' where numQ7주='0'
	update $table_m set numQ8주='' where numQ8주='0'
	update $table_m set 등록일자='' where 등록일자='0'



	update $table_m set 생년월일=left(생년월일,4)+'-'+substring(생년월일,5,2)+'-'+right(생년월일,2) where len(생년월일)=8
	update $table_m set numQ1주=left(numQ1주,4)+'-'+substring(numQ1주,5,2)+'-'+right(numQ1주,2) where len(numQ1주)=8
	update $table_m set numQ2주=left(numQ2주,4)+'-'+substring(numQ2주,5,2)+'-'+right(numQ2주,2) where len(numQ2주)=8
	update $table_m set numQ3주=left(numQ3주,4)+'-'+substring(numQ3주,5,2)+'-'+right(numQ3주,2) where len(numQ3주)=8
	update $table_m set numQ4주=left(numQ4주,4)+'-'+substring(numQ4주,5,2)+'-'+right(numQ4주,2) where len(numQ4주)=8
	update $table_m set numQ5주=left(numQ5주,4)+'-'+substring(numQ5주,5,2)+'-'+right(numQ5주,2) where len(numQ5주)=8
	update $table_m set numQ6주=left(numQ6주,4)+'-'+substring(numQ6주,5,2)+'-'+right(numQ6주,2) where len(numQ6주)=8
	update $table_m set numQ7주=left(numQ7주,4)+'-'+substring(numQ7주,5,2)+'-'+right(numQ7주,2) where len(numQ7주)=8
	update $table_m set numQ8주=left(numQ8주,4)+'-'+substring(numQ8주,5,2)+'-'+right(numQ8주,2) where len(numQ8주)=8
	update $table_m set 등록일자=left(등록일자,4)+'-'+substring(등록일자,5,2)+'-'+right(등록일자,2) where len(등록일자)=8


	update $table_m set 핸드폰='0'+핸드폰 where left(핸드폰,1)='1'

	update $table_m set 핸드폰=left(핸드폰,3)+'-'+substring(핸드폰,4,4)+'-'+right(핸드폰,4) where len(핸드폰)=11
