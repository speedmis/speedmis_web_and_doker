...
if($ServerVariables_QUERY_STRING=='') { ?>
<script>
...
//echo "https://graph.facebook.com/v2.6/me/$ServerVariables_QUERY_STRING&fields=email,name";
//echo "<br><br><br><br>";
$result = file_get_contents_new("https://graph.facebook.com/v2.6/me/$ServerVariables_QUERY_STRING&fields=email,name");


$result = json_decode($result, true);
//if(InStr($result, '"email": "')==0) exit;
//print_r($result);exit;


$google_sub = 'f' . $result['id'];
...
if(isset($result['picture'])) $google_picture = $result['picture']; else $google_picture = '';
...
if(count($result_sns)==0) {
    echo "<script>alert('기존에 등록된 페이스북 계정이 휴면상태입니다. 관리자에게 문의하세요.');</script>";
