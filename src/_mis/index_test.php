...
if ($MisSession_UserID == '') {
    if ($ServerVariables_URL == '')
        ...
        re_direct($full_site . '/_mis/login/index.php?preaddress=' . $ServerVariables_URL);
...
if ($isMenuIn == 'auto') {
    if (InStr($ServerVariables_HTTP_USER_AGENT, 'mobile') + InStr($ServerVariables_HTTP_USER_AGENT, 'mobile') > 0) {
        re_direct('index.php?' . str_replace('&isMenuIn=auto', '', $ServerVariables_QUERY_STRING), false);
    } else {
        re_direct('index.php?' . str_replace('&isMenuIn=auto', '&isMenuIn=Y', $ServerVariables_QUERY_STRING), false);
    }
}
...
if ($isMobile == 'Y' && $screenMode == '1') {
    $viewTarget = 'popup';
} else if (getCookie('viewTarget') == '') {
    if ($isMobile == 'Y') {
        $viewTarget = 'popup';
    } else {
        $viewTarget = 'right';
...
if (count($data) > 0) {
    ...
    if ($data[0]['idx'] != $gubun) {
        re_direct('index.php?gubun=' . $data[0]['idx'] . iif($isMenuIn == 'N' || $screenMode == '1', '', '&isMenuIn=Y') . $liteString, false);
    } else {
...
if ($helpbox == '') {
    ...
    if ($AddURL != '' && requestVB('isAddURL') != 'Y' && InStr('01,02,04,05,06,21,22', $MenuType) > 0) {
        re_direct('index.php?' . $ServerVariables_QUERY_STRING . $AddURL . '&isAddURL=Y');
        ...
        if ($new_ServerVariables_QUERY_STRING != $ServerVariables_QUERY_STRING || InStr($new_ServerVariables_QUERY_STRING, '@SQL:') > 0) {
            ...
            re_direct('index.php?' . $new_ServerVariables_QUERY_STRING);
        }
    }
}
...
if ($ActionFlag == 'view' && $BodyType == 'only_one_list' && $Read_Only != 'Y') {
    re_direct('index.php?' . str_replace('&ActionFlag=view', '', $ServerVariables_QUERY_STRING) . '&ActionFlag=modify', false);
...
<div class="themechooser" data-rel="themechooser" data-role="details">
    ...
    <div class="tc-theme-list k-content">
        <div class="tc-column">
            <h4>화면 색상</h4>
            <ul class="themes-grid" data-role="themechooser" data-template="theme-item-template"
                data-bind="source: themes, value: selectedTheme"></ul>
        </div>
        <div class="tc-column tb-column">
            <h4>Screen Mode</h4>
            <a href="javascript:;" onclick="setCookie('screenMode','1',1000);setCookie('pageSizes','20',1000);setCookie('viewTarget','popup',1000);
            location.href = '<?php echo $pureUrl; ?>';" class="tb-link<?php if ($screenMode == "2")
                   echo " screenModeBtn"; ?>">
                <span class="k-icon k-i-layout-2-by-2"></span>
                Mobile
            </a>
            <a href="javascript:;" onclick="setCookie('screenMode','2',1000);if(isMobile()==false) setCookie('viewTarget','right',1000);
            location.href = '<?php echo $pureUrl; ?>&isMenuIn=auto';" class="tb-link<?php if ($screenMode == '1')
                   echo " screenModeBtn"; ?>">
        ...
        <?php if ($isMobile == "N" || $screenMode != '1') { ?>
            <div class="tc-column tb-column viewTarget">
                <h4>팝업 위치: 목록 더블클릭 시, 창위치(+Ctrl key:새창)</h4>
                <a href="javascript:;"
                    onclick="setCookie('viewTarget','popup',1000);
            $(this).parent().children('a').addClass('screenModeBtn');$(this).removeClass('screenModeBtn');$('span.tc-activator.k-content.tc-active').click();"
                    class="popup tb-link<?php if ($viewTarget != "popup")
                        echo " screenModeBtn"; ?>">
                    내부팝업
                </a>
                <a title="목록에서 조회를 열때, 우측에 표시되며 상세목록을 동시에 나타내지 않습니다." href="javascript:;"
                    onclick="setCookie('viewTarget','right',1000);setCookie('rframe','',1000);location.href = location.href;
            $(this).parent().children('a').addClass('screenModeBtn');$(this).removeClass('screenModeBtn');$('span.tc-activator.k-content.tc-active').click();" class="right right1 tb-link<?php if ($viewTarget != "right" || getCookie('rframe') == '2' || getCookie('rframe') == '12')
                echo " screenModeBtn"; ?>">
                    우측1개
                </a>
                <a title="목록에서 바로 우측화면이 자동으로 열립니다." href="javascript:;"
                    onclick="setCookie('viewTarget','right',1000);setCookie('rframe','2',1000);location.href = location.href;
            $(this).parent().children('a').addClass('screenModeBtn');$(this).removeClass('screenModeBtn');$('span.tc-activator.k-content.tc-active').click();" class="right right2 tb-link<?php if ($viewTarget != "right" || getCookie('rframe') == '12' || getCookie('rframe') == '')
                echo " screenModeBtn"; ?>">
                    우측2개
                </a>
                <a title="목록에서 댓글/조회이력을 제외한 상세목록이 존재하면 바로 우측화면이 자동으로 열립니다" href="javascript:;"
                    onclick="setCookie('viewTarget','right',1000);setCookie('rframe','12',1000);location.href = location.href;
            $(this).parent().children('a').addClass('screenModeBtn');$(this).removeClass('screenModeBtn');$('span.tc-activator.k-content.tc-active').click();" class="right right12 tb-link<?php if ($viewTarget != "right" || getCookie('rframe') == '' || getCookie('rframe') == '2')
                echo " screenModeBtn"; ?>">
                    우측1~2
                </a>
                <a href="javascript:;"
                    onclick="setCookie('viewTarget','bottom',1000);
            $(this).parent().children('a').addClass('screenModeBtn');$(this).removeClass('screenModeBtn');$('span.tc-activator.k-content.tc-active').click();"
                    class="bottom tb-link<?php if ($viewTarget != "bottom")
                        echo " screenModeBtn"; ?>">
                    하단
                </a>
        ...
        <div class="tc-column tb-column License" title="SpeedMIS 온라인 고객센터 방문은 gadmin 계정으로만 가능합니다.">
            <h4>라이선스/원격지원
                <?php if ($MisSession_UserID == "gadmin") { ?>
                    | <a href="http://support.speedmis.com/cust_visit.php?<?php echo $paidKey_ucount . iif($paidKey_expireDate != '', '.' . $paidKey_expireDate, '') . '@' . $paidKey; ?>"
                        target=_blank>고객센터</a>
                <?php } ?>
            </h4>
            <?php if ($paidKey_ucount == "500") { ?>
                <a href="javascript:url='https://www.speedmis.com/_mis_speedmis/purchaseInformation.php?ucount=500&msg=<?php echo $paidKey_msg; ?>'; kendo_alert(ajax_url_return(url));"
                    class="tb-link screenModeBtn">
                    <span class="k-icon k-i-check-outline"></span>
                    엔터프라이즈버전 <?php echo $paidKey_msg; ?>
                </a>
            <?php } else if ($paidKey_ucount == "50") { ?>
                    <a href="javascript:url='https://www.speedmis.com/_mis_speedmis/purchaseInformation.php?ucount=50&msg=<?php echo $paidKey_msg; ?>'; kendo_alert(ajax_url_return(url));"
                        class="tb-link screenModeBtn">
                        <span class="k-icon k-i-check-outline"></span>
                        스탠다드버전 <?php echo $paidKey_msg; ?>
                    </a>
            <?php } else if ($paidKey_ucount == "5") { ?>
                        <a href="javascript:url='https://www.speedmis.com/_mis_speedmis/purchaseInformation.php?ucount=5&msg=<?php echo $paidKey_msg; ?>'; kendo_alert(ajax_url_return(url));"
                            class="tb-link screenModeBtn">
                            <span class="k-icon k-i-check-outline"></span>
                            무료버전 <?php
                            echo $paidKey_msg;
                            if (InStr($paidKey_msg, '오류') > 0 && $MisSession_UserID == "gadmin") { ?>
                                <script>
                                    if (isMainFrame()) toastr.error('<a href="setup.php" style="color:#fff;">인증서오류 / 모든 프로그램은 읽기전용 - 여기를 클릭하여 등록하세요</a>', "", { timeOut: 0, closeButton: false, positionClass: "toast-bottom-right" });
                                </script>
                    <?php } ?>
                        </a>
            <?php } else { ?>
                        <a href="javascript:url='https://www.speedmis.com/_mis_speedmis/purchaseInformation.php?ucount=0'; kendo_alert(ajax_url_return(url));"
                            class="tb-link screenModeBtn">
                            <span class="k-icon k-i-cancel"></span>
                    <?php if ($MisSession_UserID == "gadmin") { ?>
                                <a href="setup.php">여기를 클릭하여 등록하세요</a>
                                <script>
                                    if (isMainFrame()) toastr.error('<a href="setup.php" style="color:#fff;">미등록된제품 / 모든 프로그램은 읽기전용 - 여기를 클릭하여 등록하세요</a>', "", { timeOut: 0, closeButton: false, positionClass: "toast-bottom-right" });
                                </script>
                    <?php } else { ?>
                                미등록된 제품
                    <?php } ?>
        ...
        <div class="tc-column tb-column" style="display: block;width: 100%;text-align: right;">
            <a href="javascript:;" onclick="$('span.tc-activator.k-content.tc-active').click();" class="popup tb-link">
